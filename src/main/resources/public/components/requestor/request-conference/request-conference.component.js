(function () {
  ...
  function RequestConferenceController($window, reservationService, userService, AllowedMinBookingRequestTimeHours) {
    ...
    self.submit = () => {
      ...
      console.log("start: " + self.startAmPm);
      console.log("end: " + self.endAmPm);

      var tmpstart = starts.hours();
      if (self.startAmPm === "PM") {
        tmpstart = starts.hours() + 12;
      }
      var tmpend = ends.hours();
      if (self.endAmPm === "PM") {
        tmpend = ends.hours() + 12;
      }

      console.log("military start time: " + tmpstart);
      console.log("military end timem: " + tmpend);

      //8am-5pm
      if (!(tmpstart >= 8 && tmpend <= 17)) {
        alert("Hours must be between 8am and 5pm");
      } else {
        // set post data
        var postData = {
          user: self.user,
